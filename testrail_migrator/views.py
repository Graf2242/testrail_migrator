...
def download_backup_view(request, pk):
    backup = TestrailBackup.objects.get(pk=pk)
    redis_client = redis.StrictRedis(settings.REDIS_HOST, settings.REDIS_PORT)
    backup_data = redis_client.get(backup.name)

    if not backup_data:
        return HttpResponse("Backup not found in Redis", status=404)
...
def download_suites_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            task = download_suites_task.delay(
                project_id,
                config_dict,
            ...
            return redirect(
                reverse(
                    "plugins:testrail_migrator:task_status",
...
def upload_suites_view(request):
    ...
    return render(
        request,
        "migrator_form.html",
        {"form": form, "button_label": "Upload suites", "page_title": "Upload suites"},
    )
...
def download_milestones_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            task = download_milestone_task.delay(
                project_id,
                milestone_ids,
                config_dict,
                download_attachments,
