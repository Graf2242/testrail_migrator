...
def get_fake_mapping_for_steps(case_mappings):
    ids = TestCaseStep.objects.filter(test_case__in=case_mappings.values()).values_list(
        "id", flat=True
    )
...
def upload_task(
    ...
    with transaction.atomic():
        ...
        custom_fields_multi_select = parse_multi_select_from_tr(
            backup["custom_result_fields"]
        )
        custom_fields_labels = parse_labels_from_tr_fields(
            backup["custom_result_fields"]
        )
        ...
        if testy_project_id:
            ...
            with progress_recorder.progress_context("Creating projects"):
                project = MigratorService.create_project(
                    backup["project"], creator.service_user
                )
        ...
        for key, mapping_key in keys_with_single_mapping:
            with progress_recorder.progress_context(f"Creating {key}"):
                create_method = getattr(creator, f"create_{key}")
                mappings[key] = create_method(
                    backup[key], mappings[mapping_key], project.id
                )
        ...
        with progress_recorder.progress_context("Creating runs with plan as parent"):
            mappings["tests_parent_plan"], mappings["runs_parent_plan"] = (
                creator.create_runs(
        ...
        with progress_recorder.progress_context("Creating runs with mile as parent"):
            mappings["tests_parent_mile"], mappings["runs_parent_mile"] = (
                creator.create_runs(
        ...
        for key, parent_key, instance_type in keys:
            with progress_recorder.progress_context(f"Creating attachments for {key}"):
                file_attachments = testrail_client.get_attachments_from_list(
                    backup["attachments"][key], parent_key
                )
                mappings["attachments"].update(
                    creator.attachment_bulk_create(
                        file_attachments,
                        project,
                        mappings["users"],
                        parent_key,
        ...
        for key, parent_key, instance_type in keys:
            with progress_recorder.progress_context(f"Creating attachments for {key}"):
                file_attachments = testrail_client.get_attachments_from_list(
                    backup["attachments"][f"tests_{key}"], parent_key
                )
                mappings["attachments"].update(
                    creator.attachment_bulk_create(
                        file_attachments,
        ...
        mappings_keys = [
            ...
            (
                "results_parent_mile",
                TestResult,
                TestResultService().result_update,
                ["comment"],
            ),
            (
                "results_parent_plan",
        ...
        for mapping_key, model_class, update_method, field_list in mappings_keys:
            with progress_recorder.progress_context(
                f"Looking for attachments in fields of {mapping_key}"
            ):
...
def upload_suites_task(
    ...
    progress_recorder = ProgressRecorderContext(
        self, total=7, description="Upload started"
    )
    with transaction.atomic():
        logging.info("redis about to start")
        redis_client = redis.StrictRedis(settings.REDIS_HOST, settings.REDIS_PORT)
        ...
        with progress_recorder.progress_context("Creating suites"):
            mappings["suites"] = creator.create_suites(
                backup["suites"], testy_project_id
            )

        with progress_recorder.progress_context("Creating sections"):
            mappings["sections"] = creator.create_sections(
                backup["sections"], mappings["suites"], testy_project_id
            )
        ...
        with progress_recorder.progress_context("Creating attachments for cases"):
            file_attachments = testrail_client.get_attachments_from_list(
                backup["attachments"]["cases"], "case_id"
            )
            mappings["attachments"].update(
                creator.attachment_bulk_create(
                    file_attachments,
                    Project.objects.get(pk=testy_project_id),
                    mappings["users"],
                    "case_id",
                    mappings["cases"],
                    InstanceType.CASE,
        ...
        with progress_recorder.progress_context(
            "Looking for attachments in fields of cases"
        ):
            creator.update_testy_attachment_urls_async(
                mappings["cases"],
                TestCase,
...
def download_task(
    ...
    progress_recorder = ProgressRecorderContext(
        self, total=21, description="Download started"
    )
    ...
    with progress_recorder.progress_context("Getting custom fields for results "):
        resulting_data["custom_result_fields"] = (
            testrail_client.get_custom_result_fields()
        )
    ...
    with progress_recorder.progress_context("Getting cases"):
        resulting_data["cases"] = testrail_client.get_cases(
            project_id, resulting_data["suites"]
        )
    with progress_recorder.progress_context("Getting sections"):
        resulting_data["sections"] = testrail_client.get_sections(
            project_id, resulting_data["suites"]
        )
    ...
    with progress_recorder.progress_context("Getting milestones"):
        resulting_data["milestones"] = testrail_client.get_milestones(
            project_id, ignore_completed, query_params
        )
    with progress_recorder.progress_context("Getting plans"):
        resulting_data["plans"] = testrail_client.get_plans_with_runs(
            project_id, query_params
        )
    with progress_recorder.progress_context("Getting runs for plans"):
        resulting_data["runs_parent_plan"] = testrail_client.get_runs_from_plans(
            resulting_data["plans"]
        )
    with progress_recorder.progress_context("Getting runs for milestones"):
        resulting_data["runs_parent_mile"] = testrail_client.get_runs(
            project_id, query_params=query_params
        )
    with progress_recorder.progress_context("Getting tests for runs from plans"):
        resulting_data["tests_parent_plan"] = testrail_client.get_tests_for_runs(
            resulting_data["runs_parent_plan"]
        )
    with progress_recorder.progress_context("Getting tests for runs from miles"):
        resulting_data["tests_parent_mile"] = testrail_client.get_tests_for_runs(
            resulting_data["runs_parent_mile"]
        )
    ...
    with progress_recorder.progress_context(
        "Getting results for tests from milestones"
    ):
    ...
    if not download_attachments:
        resulting_data["failed_suites"] = list(testrail_client.failed_suites)
    ...
    for key, instance_type in keys_instance_type:
        with progress_recorder.progress_context(f"Getting attachments for {key}"):
            resulting_data["attachments"][key] = (
                testrail_client.get_attachments_for_instances(
                    resulting_data[key], instance_type
                )
            )
...
def download_milestone_task(
    ...
    progress_recorder = ProgressRecorderContext(
        self, total=14, description="Download started"
    )
    testrail_client = TestRailClient(TestrailConfig(**config_dict))
    with progress_recorder.progress_context("Getting users"):
        resulting_data["users"] = testrail_client.get_users(project_id)
    ...
    with progress_recorder.progress_context("Getting runs"):
        ...
        for milestone in resulting_data["milestones"]:
            for child_milestone in milestone["milestones"]:
                ...
                resulting_data["plans"].extend(
                    testrail_client.get_plans_with_runs(project_id, query_params)
                )
                resulting_data["runs_parent_mile"].extend(
                    testrail_client.get_runs(project_id, query_params)
                )
...
def download_suites_task(
    ...
    progress_recorder = ProgressRecorderContext(
        self, total=5, description="Download started"
    )
    ...
    with progress_recorder.progress_context("Getting suites"):
        ...
        if suite_ids:
            resulting_data["suites"] = [
                elem for elem in resulting_data["suites"] if elem["id"] in suite_ids
            ]
    with progress_recorder.progress_context("Getting cases"):
        resulting_data["cases"] = testrail_client.get_cases(
            project_id, resulting_data["suites"]
    ...
    with progress_recorder.progress_context("Getting attachments for cases"):
        resulting_data["attachments"]["cases"] = (
            testrail_client.get_attachments_for_instances(
                resulting_data["cases"], InstanceType.CASE
            )
        )
    print(f"SUMMARY OF STEPS {progress_recorder.current}")
    resulting_data["failed_suites"] = list(testrail_client.failed_suites)
...
def download_plans_runs_task(
    ...
    resulting_data = {}
    progress_recorder = ProgressRecorderContext(
        self, total=11, description="Download started"
    )
    testrail_client = TestRailClient(TestrailConfig(**config_dict))
    with progress_recorder.progress_context("Getting users"):
        resulting_data["users"] = testrail_client.get_users(project_id)
    ...
    with progress_recorder.progress_context("Getting plans"):
        resulting_data["plans"] = [
            async_to_sync(testrail_client.get_plan)(plan_id) for plan_id in plans_ids
        ]
    with progress_recorder.progress_context("Getting runs"):
        resulting_data["runs_parent_mile"] = [
            async_to_sync(testrail_client.get_run)(run_id) for run_id in runs_ids
        ]
        resulting_data["runs_parent_plan"] = testrail_client.get_runs_from_plans(
            resulting_data["plans"]
        )
...
def upload_plans_runs_task(
    ...
    progress_recorder = ProgressRecorderContext(
        self, total=13, description="Upload started"
    )
    with transaction.atomic():
        logging.info("redis about to start")
        redis_client = redis.StrictRedis(settings.REDIS_HOST, settings.REDIS_PORT)
