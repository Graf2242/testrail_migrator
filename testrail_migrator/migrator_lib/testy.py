...
class TestyCreator:
    ...
    @staticmethod
    def get_steps(custom_steps_tr: List[Dict[str, str]]):
        steps_info = {'is_steps': True, 'steps': []}
        for idx, step in enumerate(custom_steps_tr, start=1):
            testy_step = dict()
            name = step.get('content').split('\n')[0] if step.get('content', '').split('\n')[0] else f'Step {idx}'
            testy_step['name'] = name
            testy_step['scenario'] = f'{step.get("content", "Scenario was not provided")}\n  '
            if additional_info := step.get('additional_info'):
                testy_step['scenario'] += f'Additional info:\n  {additional_info}\n  '
            if refs := step.get('refs'):
                testy_step['scenario'] += f'References:\n  {refs}\n  '
            testy_step['expected'] = step.get('expected', '')
            testy_step['sort_order'] = idx
            steps_info['steps'].append(testy_step)
        return steps_info

    # TODO: use default case service to create cases with steps think about bulk creation
    def create_cases(self, cases, suite_mappings, section_mappings, project_id, custom_fields_matcher: dict):
        cases_data_list = []
        src_case_ids = []
        created_cases = []
        for case in cases:
            ...
            custom_fields = self.parse_case_custom_fields(case, custom_fields_matcher)
            case_data.update(custom_fields)
            if case.get('custom_steps_separated'):
                case_data.update(self.get_steps(deepcopy(case['custom_steps_separated'])))
            cases_data_list.append(case_data)
            with suppress_auto_now(TestCase, ['created_at', 'updated_at']):
                if case_data['is_steps']:
                    created_case = MigratorService().case_with_steps_create(case_data)
                else:
                    created_case = MigratorService().case_create(case_data)
                created_cases.append(created_case)
        return dict(zip(src_case_ids, [created_case.id for created_case in created_cases]))

    def parse_case_custom_fields(self, tr_case_dict: dict, custom_fields_matcher: dict):
        case_data = {}
        for tr_field, testy_field in custom_fields_matcher.items():
            tr_field_content = tr_case_dict.get(tr_field)
            if tr_field_content is None:
                continue
            if testy_field_content := case_data.get(testy_field):
                case_data[testy_field] = f'{testy_field_content}{self.markdown_endl_sep}' \
                                         f'{tr_field}:{self.markdown_endl_sep}{tr_field_content}'
                continue
            case_data[testy_field] = str(tr_field_content)
        scenario = case_data.get('scenario')
        if not scenario or scenario.isspace():
            case_data['scenario'] = 'Scenario was not provided'
        return case_data
    ...
    def create_configs(config_groups, project_id):
        ...
        for parameter_data in parameter_data_list:
            parameter, _ = Parameter.objects.get_or_create(**parameter_data)
    ...
    @staticmethod
    def create_plans(plans, milestones_mappings, project_id, skip_root_plans: bool = True, force_parent: bool = False,
                     force_parent_id: int = None):
        plan_data_list = []
        plan_mappings = {}
        src_plan_ids = []
        for plan in plans:
            ...
            if force_parent:
                plan_data['parent'] = TestPlan.objects.get(pk=force_parent_id) if force_parent_id else None

            if mapping_id and not force_parent:
    ...
    def create_runs(runs, mapping, config_mappings, tests, case_mappings, project_id,
        ...
        for idx, run in enumerate(runs, start=1):
            ...
            parent = force_parent_id if parent_type == ParentType.FORCE_PARENT else mapping.get(run[parent_id_key])
    ...
    def get_step_results(self, test: Test, custom_steps_results):
        parsed_steps = []
        steps = TestCaseStep.objects.filter(test_case=test.case)
        for step, testy_step in zip(custom_steps_results, steps):
            parsed_steps.append(
                {
                    'status': self.statuses_mapping.get(step['status_id'], 5),
                    'step': testy_step
                }
            )
        return parsed_steps
    ...
    def create_results(self, results, custom_fields_multi_select, custom_fields_labels, tests_mappings, user_mappings):
        ...
        for idx, result in enumerate(results):
            ...
            if not tests_mappings.get(result['test_id']):
                continue
            # Drop all results that serve as assignation message or comment messages
            ...
            for result_field_name, result_field_value in result.items():
                if not re.match(r'^custom_', result_field_name) or not result_field_value:
                    continue
                if result_field_name == 'custom_step_results':
                    continue
            ...
            result_data = {
                'status': self.statuses_mapping.get(result['status_id'], 5),
                'test': Test.objects.get(pk=tests_mappings[result['test_id']]),
                'created_at': timezone.make_aware(datetime.fromtimestamp(result['created_on'])),
                'updated_at': timezone.make_aware(datetime.fromtimestamp(result['created_on'])),
                'attributes': json_fields
            }

            if result.get('custom_step_results'):
                result_data['steps_results'] = self.get_step_results(result_data['test'],
                                                                     result['custom_step_results'])

            if comment := result.get('comment'):
                result_data['comment'] = comment

            if defects := result.get('defects'):
                if comment := result_data.get('comment'):
                    result_data['comment'] = comment + f'\n Defects: {defects}'
                else:
                    result_data['comment'] = f'Defects: {defects}'
